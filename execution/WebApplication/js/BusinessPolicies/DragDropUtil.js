/************************************************************************************************************
    *** MODIFIED VERSION FOR BUSINESS POLICIES ***
...
DHTMLgoodies_common.prototype = {
	...
	getTopPos : function(inputObj)
	{
		try  {
			...
			window.status = "getTopPos : " + returnValue;
			return returnValue;
	...
	getLeftPos : function(inputObj)
	{
	  try  {
		...
		window.status = "getLeftPos : " + returnValue;
		return returnValue;
	...
	// }}}	
	,
	copyBorderStyle : function(object1, style2){
		if (object1 != null) {

			object1.style.borderLeftColor = style2.borderLeftColor;
			object1.style.borderTopColor = style2.borderTopColor;
			object1.style.borderRightColor = style2.borderRightColor;
			object1.style.borderBottomColor = style2.borderBottomColor;
			object1.style.borderTopStyle = style2.borderTopStyle;
			object1.style.borderLeftStyle = style2.borderLeftStyle;
			object1.style.borderBottomStyle = style2.borderBottomStyle;
			object1.style.borderRightStyle = style2.borderRightStyle ;
			object1.style.borderTopWidth = style2.borderTopWidth;
			object1.style.borderRightWidth = style2.borderRightWidth;
			object1.style.borderBottomWidth = style2.borderBottomWidth;
			object1.style.borderLeftWidth = style2.borderLeftWidth;
		}
	}
}
...
function DHTMLgoodies_dragDrop()
{
	...
	var cloneFunction;
	var resolveDestinationFunction;
	var currentDropTarget;
	var lastDropTarget;
	var originalElementStyle;
	...
	var dragdropElement = document.getElementById("DragDropInternal");
    if (dragdropElement == null){
        dragdropElement = document.createElement("div");
        dragdropElement.id='DragDropInternal';
        document.body.appendChild(dragdropElement);
    } else  {
...
DHTMLgoodies_dragDrop.prototype = {
     ...
     * @public
     */	
	init : function(cloneFunction, resolveDestinationFunction)
	{
		if(!standardObjectsCreated)DHTMLgoodies_createStandardObjects();	// This line starts all the init methods
		this.cloneFunction = cloneFunction;
		this.resolveDestinationFunction = resolveDestinationFunction;
	...
	__initDragDropScript : function()
	{
		...
		for(var no=0;no<this.dragDropSourcesArray.length;no++){
			...
			if (this.cloneFunction != null)
				el = cloneFunction(this.dragDropSourcesArray[no][0]);
			else
				el = this.dragDropSourcesArray[no][0].cloneNode(true);
			...
			dragdropElement.appendChild(el);
	...
	__initDragDropElement : function(e)
	{
		...
		referenceToDragDropObject.el_x = parseInt(referenceToDragDropObject.dragObjCloneArray[referenceToDragDropObject.numericIdToBeDragged].style.left.replace('px',''));
		referenceToDragDropObject.el_y = parseInt(referenceToDragDropObject.dragObjCloneArray[referenceToDragDropObject.numericIdToBeDragged].style.top.replace('px',''));
		
		referenceToDragDropObject.currentDropTarget = null;
	...
	__moveDragableElement : function(e)
	{
		...
		if (dragObj.parentNode.tagName == "DL" || dragObj.parentNode.tagName == "LI" || dragObj.parentNode.tagName == "UL"){
			dragObj.parentNode.parentNode.appendChild(dragObj);
		...
		if (referenceToDragDropObject.resolveDestinationFunction != null)
			dropDestination = resolveDestinationFunction(dropDestination);
		
		if (referenceToDragDropObject.lastDropTarget != dropDestination){
			
			dhtmlSuiteCommonObj.copyBorderStyle(referenceToDragDropObject.lastDropTarget, referenceToDragDropObject.originalElementStyle);
			referenceToDragDropObject.lastDropTarget = dropDestination;
			if (dropDestination != null) {
				
				referenceToDragDropObject.originalElementStyle =  clone(referenceToDragDropObject.lastDropTarget.style);
				referenceToDragDropObject.lastDropTarget.style.border = '1px solid #CC0000';
			}
		}
				
		if (dropDestination != null)
			referenceToDragDropObject.currentDropTarget = dropDestination;
		else
			referenceToDragDropObject.currentDropTarget = null;
	}
	// }}}	
	...
	__stop_dragDropElement : function(e)
	{
		...
		dropDestination = referenceToDragDropObject.currentDropTarget;
		dhtmlSuiteCommonObj.copyBorderStyle(referenceToDragDropObject.lastDropTarget, referenceToDragDropObject.originalElementStyle);
		...
		if (dropDestination != null) {
			...
			for(var no=0;no<referenceToDragDropObject.dragDropTargetArray.length;no++){
				if (referenceToDragDropObject.dragDropTargetArray[no][0].id == dropDestination.id)	{
	...
	__slideElementBackIntoItsOriginalPosition : function(numId)
	{
		...
		var currentX = parseInt( this.dragObjCloneArray[numId].style.left.replace('px','') );
		var currentY = parseInt( this.dragObjCloneArray[numId].style.top.replace('px','') );
		...
		//self.status = "currentX: " + currentX + "currentY: " + currentY +  "targetX: " + targetX + "targetY:" + targetY;
