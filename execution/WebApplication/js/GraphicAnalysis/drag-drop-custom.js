...
DHTMLgoodies_common.prototype = {
	...
	{
		/*var position = findPos(inputObj);
		var parentPosition = null;
		
		if (clientInfoObj.isMSIE && inputObj.parentNode.tagName == "DL"){
			parentPosition = findPos(inputObj.parentNode)[1];
			self.status = "getTopPos : " + position[1] + ", parentPosition: " + parentPosition;
			return position[1] - parentPosition;
		}
	  ...
	  //self.status = "getTopPos : " + returnValue;
	  return returnValue;
	}
	// }}}
	...
	{
	/*	var position = findPos(inputObj);
		self.status = 'getLeftPos : ' + position[0];
		var parentPosition = null;
		
		if (clientInfoObj.isMSIE && inputObj.parentNode.tagName == "DL"){
			parentPosition = findPos(inputObj.parentNode)[0];
			return position[0] - parentPosition;
		}
