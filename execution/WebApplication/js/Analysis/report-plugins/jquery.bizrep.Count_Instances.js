 ...
 * 2- Apply plugin passing the options object      e.g: $(selector).BizRep_Count_Instances(myOptions);
...
(function ($) {
    ...
    var PLUGIN_TEMPLATE_FILE = "Rep_Tpt_CounterInstances.aspx";

    /* Report name to be shown in error messages" */
    var PLUGIN_NAME = "Count_Instances";

    $.fn.extend({


        BizRep_Count_Instances: function (options) {
            ...
            function DrawReport(oJsonResult) {
                ...
                try {
                    ...
                    //Fix column labels
                    FixBarDisplayValues(oResultTable);
                    ...
                    oChartOptions.catNameColumnIndex = 0;
            ...
            function FixBarDisplayValues(oResultTable) {

                var iColumnInstances = 1;

                var iTotalCases = oResultTable.extendedProperties && oResultTable.extendedProperties.totalCases ? parseInt(oResultTable.extendedProperties.totalCases) : 0;

                //Apply format for all rows
                for (iRow = 0; iRow < oResultTable.rows.length; iRow++) {
                    var sDisplayInstances = parseInt(oResultTable.rows[iRow][iColumnInstances].value);
                    var sDisplayPercent = 0;

                    if (iTotalCases > 0) {
                        var iFraction = parseInt(oResultTable.rows[iRow][iColumnInstances].value) / iTotalCases;
                        sDisplayPercent = (100 * iFraction);
                    }

                    oResultTable.rows[iRow][iColumnInstances].text  += " (" + sDisplayPercent.toFixed(0) + " %25)";
                }
            }
        ...
        BizRep_Count_Instances_ChartClick: function (clickParams  /* {reportIndex, rowClicked, colClicked} */) {
